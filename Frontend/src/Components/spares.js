...
export default function Spares() {
  ...
  useEffect(() => {
    axios.get("http://localhost:5005/getspares")
      .then((response) => {
  ...
  const addToCart = (item) => {
    ...
    axios
      .post('http://localhost:5005/api/addToCart', { ...item, username, formattedDate })
      .then((response) => {
        setCart([...cart, response.data, formattedDate]);
        console.log([...cart, response.data, formattedDate])
